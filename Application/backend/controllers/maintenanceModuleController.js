...
const getLateFeeRule = async (req, res) => {
  try {
    const [rows] = await promisePool.query(
      'SELECT * FROM late_fee_rules WHERE active = TRUE ORDER BY id DESC LIMIT 1'
    );
    return respond(res, 200, 'Late fee rule fetched', rows[0] || null);
  } catch (error) {
    // Keep a database failure scoped to this request; never let it become an
    // unhandled rejection that can take down the Railway process.
    console.error('Get late fee rule error:', error);
    return respond(res, 500, 'Unable to fetch late fee rule');
  }
};
