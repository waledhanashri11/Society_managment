...
const Reports = () => {
  ...
  const matchesMonthYear = (value, fallbackMonth, fallbackYear) => {
    ...
    const rowMonth = date && !Number.isNaN(date.getTime()) ? date.getMonth() + 1 : Number(fallbackMonth || 0);
    const rowYear = date && !Number.isNaN(date.getTime()) ? date.getFullYear() : Number(fallbackYear || 0);
  ...
  const filteredBills = useMemo(() => {
    return bills.filter((bill) => {
      if (!matchesMonthYear(bill.due_date || bill.payment_date, bill.month, bill.year)) return false;
      if (filters.status && statusKey(bill.payment_status || bill.status) !== statusKey(filters.status)) return false;
      return true;
    });
  }, [bills, filters]); // eslint-disable-line react-hooks/exhaustive-deps

  const filteredComplaints = useMemo(() => {
    return complaints.filter((complaint) => matchesMonthYear(complaint.created_at));
  }, [complaints, filters]); // eslint-disable-line react-hooks/exhaustive-deps

  const filteredExpenses = useMemo(() => {
    return expenses.filter((expense) => matchesMonthYear(expense.expense_date || expense.date));
  }, [expenses, filters]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const downloadCsv = () => {
    const rows = [
      ...
      ...filteredExpenses.map((expense) => [
        ...
        expense.category, expense.amount, fullDate(expense.expense_date || expense.date), expense.description
      ]),
      [],
      ['Complaints'],
      ['Resident', 'Title', 'Status', 'Date'],
      ...filteredComplaints.map((complaint) => [complaint.user_name || complaint.resident_name, complaint.title, complaint.status, fullDate(complaint.created_at)])
    ];
  ...
  const downloadPdf = () => {
    const html = `
      <html><head><title>Admin Reports</title><style>
      ...
      <tbody>${filteredExpenses.map((expense) => `<tr><td>${expense.vendor || expense.expense_title || ''}</td><td>${expense.category || ''}</td><td>${money(expense.amount)}</td><td>${fullDate(expense.expense_date || expense.date)}</td></tr>`).join('')}</tbody></table>
    ...
    <div className="portal-module">
      ...
      <div className="portal-dashboard-grid">
        <section className="portal-panel portal-table-card">
          <div className="portal-panel-head"><div><h2>Expenses Report</h2><p>Society expense records.</p></div></div>
          <div className="portal-table-wrap">
            <table className="portal-data-table">
              <thead><tr><th>Expense Title</th><th>Category</th><th>Amount</th><th>Date</th><th>Description</th></tr></thead>
              <tbody>{filteredExpenses.map((expense) => <tr key={expense.id}><td><strong>{expense.vendor || expense.expense_title || expense.expense_number}</strong></td><td>{expense.category}</td><td>{money(expense.amount)}</td><td>{fullDate(expense.expense_date || expense.date)}</td><td>{expense.description || <span className="portal-muted-text">No description</span>}</td></tr>)}</tbody>
            </table>
            {!filteredExpenses.length && <div className="portal-empty">No report data found.</div>}
          </div>
        </section>
