              ...
              {bills.length ? <div className="mm-list">
                {bills.slice(0, 5).map((bill) => (
                  <div className="mm-list-row" key={bill.id}>
                    ...
                    <div className="mm-list-amount"><strong>{money(bill.total_amount)}</strong><span className={statusClass(bill.payment_status || bill.status)}>{statusLabel(bill.payment_status || bill.status)}</span></div>
                  </div>
                ))}
