...
/**
 * All meeting tables are created and maintained by database migrations
 * (025_meeting_management.sql, 028_meeting_management_enhancements.sql,
 *  036_meeting_runtime_schema_repair.sql, 041_multi_tenant_foundation.sql).
 *
 * This function is intentionally a no-op — it exists only so the route
 * middleware that calls it does not need to be changed. Previously it ran
 * CREATE TABLE / ALTER TABLE / CREATE INDEX on every HTTP request, which
 * failed once the restricted `society_tenant_app` role was introduced by
 * migration 041 (that role has DML but not DDL privileges).
 */
const ensureMeetingRuntimeSchema = async () => {
  // No-op: schema is managed exclusively by migrations.
};
