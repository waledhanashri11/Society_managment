...
const ensureMaintenanceRuntimeSchema = async () => {
  ...
  // Tables are created by migrations: maintenance, payments, payment_status_history,
  // maintenance_writeoffs, maintenance_audit_logs, payment_maintenance.
  // The society_tenant_app role cannot run DDL.
};
...
const createManualBill = async (req, res) => {
  try {
    ...
    if (!title || !String(title).trim()) {
      return sendResponse(res, 400, 'Bill title is required');
    }
    ...
    const billTitle = String(title).trim();
    let categoryVal = category && String(category).trim() ? String(category).trim() : 'Other';
    if (categoryVal === 'Other' && customCategory && String(customCategory).trim()) {
    ...
    return sendResponse(res, 201, 'Manual bill created successfully', { id: insertedId });
  } catch (error) {
...
const getBillById = async (req, res) => {
  try {
    ...
    const previousOutstanding = bills[0].is_manual || bills[0].bill_type === 'manual'
      ? 0
      : Number(outstandingRows[0]?.previous_outstanding || 0);
...
const createPayment = async (req, res) => {
  ...
  try {
    ...
    // A resident only needs to upload payment proof. Keep transaction_id populated
    // for legacy database constraints and admin reporting with an internal reference.
    const suppliedTransactionId = String(utrNumber || transactionId || '').trim();
    const transactionReference = suppliedTransactionId
    ...
    if ((!billId && !requestedBillIds.length) || !amount) {
      return sendResponse(res, 400, 'Bill ID and amount are required');
    }
    ...
    const [paymentRows] = await connection.query('SELECT id, payment_status FROM payments WHERE transaction_id = ?', [transactionReference]);
    ...
    const insertValues = [primaryBillId, paymentMethod, transactionReference, paidAmount, 'Pending Verification', paidAt, screenshotPath];
